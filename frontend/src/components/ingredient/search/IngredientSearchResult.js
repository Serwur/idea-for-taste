...
function render(searchState, { lastSearchName, result }, listItemProps) {
    switch (searchState) {
        ...
        case SEARCH_STATE.ERROR_FETCH:
            return renderError(
                "Ups...we cannot establish connection, please try again later =)"
            );
        default:
...
function renderError(errorMessage) {
    return (
        <div className="alert alert-danger ml-2 mr-2 p-1">{errorMessage}</div>
    );
}
    ...
    return (
        <>
            <div className="alert alert-success">
                Found {results.length} results for: '{inputIngrName}'
            </div>
            <IngredientList
                ingredients={results}
